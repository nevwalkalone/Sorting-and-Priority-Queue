...
public class AlgorithmComp {
	public static void main(String[] args) throws IOException {
		...
		for (int i = 1; i <= 20; i++, text_file_number++) {
			...
			if (comp < 0) {
				System.out.println(
						"\nComparison:Greedy Algorithm is more efficient than Greedy-Decreasing Algorithm in TEXTFILE("
								+ text_file_number + ")");
			} else if (comp > 0) {
				System.out.println(
						"\nComparison:Greedy-Decreasing Algorithm is more efficient than Greedy Algorithm in TEXTFILE("
								+ text_file_number + ")");
			} else {
				System.out.println(
		...
		System.out.println("\nAverage of disk files used in all text files by Greedy Algorithm: "
				+ ((double) Total_Disks_Greedy / 20));
		System.out.println("Average of disk files used in all text files by Greedy-Decreasing Algorithm: "
				+ ((double) Total_Disks_Decreasing / 20));
